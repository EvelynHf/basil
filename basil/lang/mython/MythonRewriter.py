...
class MyRewriter (ASTHandler):
    ...
    def handle_QuoteDef (self, node):
        ...
        self.env = env
        # XXX Do some sanity checks on the result of the node
        # expansion (should be a list of stmt AST nodes and should not
        # contain any QuoteDef nodes).
        if (len(ret_val) > 0) and ("QUOTEDEF_AST_DEBUG_FLAG" in env):
            import pprint
            import ASTUtils
            print("_" * 70)
            print("AST GENERATED BY %r FROM CODE ON LINE %d" %
                  (quotefn, node.body_ofs[0]))
            ast_to_tuple = ASTUtils.mk_ast_to_tuple(myfront_ast.AST)
            pprint.pprint([ast_to_tuple(ast) for ast in ret_val])
            print("_" * 70)
        return ret_val
    # ____________________________________________________________
